...
TEST(MatrixFilling, NoMatrix) {
    EXPECT_EQ(fill_matrix(nullptr), INVALID_DATA);
}
...
TEST(MatrixFilling, NoData) {
    ...
    EXPECT_EQ(fill_matrix(test_matrix), INVALID_DATA);
...
TEST(MatrixFilling, CorrectData) {
    ...
    freopen("test_input.txt", "r+", stdin);
    ASSERT_EQ(fill_matrix(test_matrix), 0);
...
TEST(MatrixSorting, NoMatrix) {
    EXPECT_EQ(sort_matrix(nullptr), nullptr);
}
